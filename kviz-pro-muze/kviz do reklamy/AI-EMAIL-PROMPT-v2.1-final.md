# AI Email Prompt — kvíz po vyhodnocení (v2.1 · FINÁLNÍ)

> Verze 2.1 / 2026-07-09 · Cliqsales AI → personalizovaný e-mail výsledku
>
> CTA tlačítko je DYNAMICKÉ — AI si samo vybere jednu ze 3 pevných URL podle
> toho, který TEXT (1–6) použilo pro diagnózu.
>
> 📋 Kopíruj do Cliqsales celý text od „# ÚKOL" dolů (markdown nadpisy modelu nevadí).
>
> Tento prompt patří k NOVÉMU kvízu na restartmuze.cz/kviz-pro-muze/ (kopie
> widgetu v Cliqsales, samostatné quiz ID) — starý kvíz na /kviz-vztahu/ a jeho
> automatizace zůstávají beze změny.
>
> URL cíle:
> - TEXT 1 + TEXT 3 → Cliqsales trigger link `{{trigger_link.iREeRlRexylEfNgByXVJ}}`
>   (vede na jak-zachranit-vztah/ — webinář Restart vztahu — 4 kroky)
> - TEXT 2 + TEXT 4 → Cliqsales trigger link `{{trigger_link.zz35KbBZqtr44KeKWrUY}}`
>   (vede na webinare/skoncuj-s-hodnym-klukem/)
> - TEXT 5 → Cliqsales trigger link `{{trigger_link.tDJbARokXIT16xh4KvgH}}`
>   (vede na koch-zaklad/ — Lenka takto může kontakty tagovat podle prokliku)
> - TEXT 6 → 2 pevná tlačítka přímo ve zdroji: sdílení kvízu kamarádovi
>   (`kviz-pro-muze/?ref=...` s UTM) + KOCH růstová varianta „Dozvědět se víc
>   o KOCHu →" přes trigger link `{{trigger_link.uQY27LV2DNIss7bjZ9QF}}`
>   (vede na koch-zaklad3/ — „základy máš, pojď růst a předávat", NE diagnóza)
>
> Změna 2026-07-15 (2): i TEXT 1–4 přecházejí na Cliqsales trigger linky
> (RV4 = iREeRlRexylEfNgByXVJ, HK = zz35KbBZqtr44KeKWrUY) — všechna dynamická
> tlačítka teď jdou přes trigger linky a Lenka na proklik věší tagy
> v Cliqsales. Sdílecí odkaz v TEXTU 6 dostal UTM parametry.
>
> Změna 2026-07-15: TEXT 6 už NEMÁ tlačítko na knihu Restart muže (Sláva:
> zůstávají jen 2 tlačítka — sdílej kamarádovi + Dozvědět se víc o KOCHu,
> proto pro knihu není trigger link). Text KOCH tlačítka přejmenován
> z „Nakouknout do KOCHu" na „Dozvědět se víc o KOCHu" — cíl je prodejka,
> ne náhled do komunity, název musí odpovídat tomu, co po kliknutí přijde.
>
> Změna 2026-07-14: TEXT 5 a KOCH tlačítko v TEXTU 6 používají Cliqsales
> trigger linky místo pevné URL s UTM — Lenka tak může na proklik navázat
> tagování kontaktu přímo v Cliqsales. Ostatní tlačítka (TEXT 1–4)
> zůstávají na pevných URL s UTM parametry.
>
> Změna 2026-07-11: TEXT 6 nevede na KOCH přes dynamický VSTUP C (ten platí jen
> pro TEXT 1–5). Samé-C respondent nemá diagnózu, proto ho TEXT 6 zve jinak:
> pozvánka do KOCHu jako do prostředí, kde poroste dál a předá, co umí.
> Obě tlačítka jsou napevno ve zdroji TEXTU 6 — AI nic nevybírá.
...
<p>Ahoj chlape,</p>
<p>vypadá to, že se Tvoje odpovědi z kvízu správně nenačetly. Vyplň ho prosím ještě jednou tady: <a href="https://restartmuze.cz/kviz-pro-muze/">restartmuze.cz/kviz-pro-muze</a>, ať Ti pošlu přesné vyhodnocení.</p>
<p>Sláva Černý</p>
...
Pro TEXT 6 → NEVKLÁDEJ VSTUP A, VSTUP B ani VSTUP C (žádné P.S.). Všechna
tlačítka TEXTU 6 (sdílení kvízu, KOCH, kniha) už jsou napevno v jeho zdroji,
včetně podpisu na konci. Nic nenahrazuj, nepřidávej ani nevybírej — TEXT 6
převezmi celý tak, jak je napsaný.
...
## VSTUP B
Kam: těsně před přechodovou větu na konci textu (u TEXTU 5 před větu
"Právě proto jsem vytvořil KOCH Základ.").
...
## VSTUP C — DYNAMICKÉ TLAČÍTKO (platí pro TEXT 1–5)
Kam: za Slávův závěrečný text.

Podle toho, který TEXT (1–5) jsi použil pro diagnózu, vyber JEDNU z těchto
dvojic URL + text tlačítka:

- Pro TEXT 1 a TEXT 3 použij:
  URL = `{{trigger_link.iREeRlRexylEfNgByXVJ}}`
  Text tlačítka = `Chci na webinář zdarma →`
- Pro TEXT 2 a TEXT 4 použij:
  URL = `{{trigger_link.zz35KbBZqtr44KeKWrUY}}`
  Text tlačítka = `Chci na webinář zdarma →`
- Pro TEXT 5 použij:
  URL = `{{trigger_link.tDJbARokXIT16xh4KvgH}}`
  Text tlačítka = `Chci vědět víc o členství →`

(TEXT 6 dynamické tlačítko nemá — jeho dvě pevná tlačítka jsou přímo
ve zdroji, viz KROK 3.)
...
DŮLEŽITÉ: před tlačítko NEVKLÁDEJ prázdný odstavec <p>&nbsp;</p> — tlačítko
následuje HNED za posledním odstavcem textu:

<p align="center" style="text-align:center;margin:8px 0 24px;">
...
- Za každou tematickou sekcí přidej <p>&nbsp;</p> — ALE NIKDY ne těsně před
  tlačítko (CTA) ani těsně za něj; tlačítka mají vlastní odsazení v margin
...
Je to roční členství v Klubu odvážných chlapů. Dostaneš chlapské prostředí,
kde se řeší skutečné věci — ne hospodské kecy. Tréninky a nástroje na
komunikaci, hranice a mužskou pevnost. Pravidelné živé vedení, abys na to
nebyl sám. A zrcadlo od chlapů, kteří ti pomůžou vidět vlastní slepá místa.
...
→ zralý muž: sdílení kvízu + nakouknutí do KOCHu (růst, ne oprava) + kniha
...
<a href="https://restartmuze.cz/kviz-pro-muze/?ref={{contact.id}}&amp;utm_source=kviz&amp;utm_medium=email&amp;utm_campaign=sdileni&amp;utm_content=text6" style="display:inline-block;background:#002338;color:#ffffff;font-family:Montserrat,Arial,sans-serif;font-weight:700;font-size:20px;text-decoration:none;padding:16px 40px;border-radius:100px;letter-spacing:0.04em;"><span style="color:#ffffff;">Pošli kvíz kamarádovi →</span></a>
</p>

<p>Tvůj výsledek ukazuje, že víš, jak budovat zdravý vztah.</p>

<p>A ještě jedna věc, tentokrát pro Tebe.</p>

<p>Pevné základy neznamenají, že není kam růst. Jestli chceš na sobě pracovat
dál — a předávat, co umíš, chlapům, kteří tak daleko ještě nejsou — podívej se,
jak funguje <strong>KOCH, Klub odvážných chlapů</strong>. Chlap jako Ty tam má
co získat i co dát.</p>

<p align="center" style="text-align:center;margin:8px 0 24px;">
<a href="{{trigger_link.uQY27LV2DNIss7bjZ9QF}}" style="display:inline-block;background:#3f9626;color:#ffffff;font-family:Montserrat,Arial,sans-serif;font-weight:700;font-size:20px;text-decoration:none;padding:16px 40px;border-radius:100px;letter-spacing:0.04em;"><span style="color:#ffffff;">Dozvědět se víc o KOCHu →</span></a>
</p>
